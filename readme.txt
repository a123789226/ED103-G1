...
由於舊有member內的memPic都是"檔案名稱"
更新版為"image/memPic/檔案名稱"
所以重新註冊一個帳號做測試

--註冊
上傳大頭照的同時，檔案就會上傳到資料夾(不過demo時候看不出來就好)。
檔案名稱會跟上傳的檔案名稱相同(demo時不要上傳一樣名稱的圖片上去就好)。
帳號密碼長度錯誤的紅色字提示，在重新填寫正確長度後，仍會存在。
紅色字提示跟帳號註冊成功的文字，中文換成英文。

--會員中心
無法更換圖片(但console也沒跳甚麼錯誤提示)
我有看memBigPhoto.php，假如更換apple.jpg上去應該會變成Mark123-apple.jpg之類的，但是他沒出現，資料庫也沒做更新QQ

無法修改會員資料
在input框打上新的會員資料，按下confirm會沒反應(console有跳錯誤)。

--其他
有些頁面是用memLogin.js轉存，所以還有幾隻跟memLogin.js很像的js檔。
裡面的
  $id('memberPic').src = `./${member.memPic}`;
都要換成你改得這樣(等你那邊全都確認後，禮拜一再一起修改)。
